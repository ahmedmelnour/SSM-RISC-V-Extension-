...
def main():
    ...
    # model_v3.pt, not model_v2.pt: v2 is the two-variable run and still carries
    # the final LayerNorm (norm_out.*) that was later reverted from model.py, so
    # loading it raises "Unexpected key(s) in state_dict" and the guard never runs.
    # v3 is also the checkpoint fw/model_data.h was exported from.
